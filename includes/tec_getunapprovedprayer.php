...
/* Query unapproved prayer listing: visible = 3 (all) and approved = 0 */
		...
		while($unapprovedrow = $unapprovedprayerresult->fetch_assoc()) {
			...
			$prayemail = "<td>" . $unapprovedrow['pray_email_addr'] . "</td>";	
			$praypraise = "<td>" . $unapprovedrow['praypraise'] . "</td>";
			$prayer_title = "<td>" . $unapprovedrow['prayertitle'] . "</td>";
			$approve_button = "<td><button type='button' class='btn btn-success prayer_approve btn-sm' href='#'>Approve</button></td>";
			$reject_button = "<td><button type='button' class='btn btn-danger prayer_reject btn-sm' href='#'>Reject</button></td>";
			$view_button = "<td><button type='button' class='btn btn-primary prayer_view btn-sm' href='#'>View</button></td>";
			$prayer_text = "<td>" . $unapprovedrow['prayertext'] . "</td></tr>";
		
			// Stores each database record to an array 
			$buildjson = array($praycontrol, $prayerid, $prayerupdate, $fullname, $prayemail, $praypraise, $prayer_title, $approve_button, $reject_button, $view_button, $prayer_text); 
